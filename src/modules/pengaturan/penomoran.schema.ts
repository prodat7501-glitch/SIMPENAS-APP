...
export const documentTypes = ["Nota Dinas", "SPT", "SPPD", "SPBY"] as const;
...
export const penomoranSchema = z.object({
  ...
  format: z.string().min(5, "Format wajib diisi").refine((v) => v.includes("{RUNNING}"), "Format wajib memiliki {RUNNING}").refine((v) => v.includes("{YEAR}"), "Format wajib memiliki {YEAR}"),
  prefix: z.string().max(60),
  suffix: z.string().max(60),
  year: z.number().int().min(2020).max(2100),
  runningNumber: z.number().int().min(0).max(999999),
