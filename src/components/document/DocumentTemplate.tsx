...
type DocumentTemplateStyle = CSSProperties & {
  "--document-print-padding": string;
};

const getDocumentTemplateStyle = (
  margin: number,
  font: string,
): DocumentTemplateStyle => ({
  padding: `${margin}mm`,
  fontFamily: font,
  "--document-print-padding": `${margin}mm`,
});
...
export function DocumentTemplate({ children, includeHeader = true }: { children: ReactNode; includeHeader?: boolean }) {
  const template = useDocumentTemplate();
  const style: DocumentTemplateStyle = { ...getDocumentTemplateStyle(template.margin, template.font), textAlign: "left" };
...
export function useTemplateDocumentStyle(): CSSProperties {
  const template = useDocumentTemplate();
  return getDocumentTemplateStyle(template.margin, template.font);
}
